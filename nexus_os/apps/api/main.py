...
def create_object(object_data: schemas.ObjectInstanceCreate, db: Session = Depends(get_db)):
    ...
    # Validate Properties against Definition
    if obj_type.property_definitions:
        definitions = {p["name"]: p for p in obj_type.property_definitions}
        for prop_name, prop_value in object_data.properties.items():
            if prop_name in definitions:
                expected_type = definitions[prop_name]["type"]
                # Basic Type Checking
                if expected_type == "integer" and not isinstance(prop_value, int):
                     raise HTTPException(status_code=400, detail=f"Property '{prop_name}' must be an integer.")
                elif expected_type == "float" and not isinstance(prop_value, (float, int)):
                     raise HTTPException(status_code=400, detail=f"Property '{prop_name}' must be a float.")
                elif expected_type == "string" and not isinstance(prop_value, str):
                     raise HTTPException(status_code=400, detail=f"Property '{prop_name}' must be a string.")
                elif expected_type == "boolean" and not isinstance(prop_value, bool):
                     raise HTTPException(status_code=400, detail=f"Property '{prop_name}' must be a boolean.")
...
def aip_chat(query: Dict[str, str] = Body(...)):
    ...
    # Smart logic to detect Station Alpha failure and recommend action.
    
    # 1. Check for Critical Incidents
    if "Station Alpha" in user_prompt or "critical" in user_prompt.lower():
        ai_text = (
            "🚨 **CRITICAL ALERT: Station Alpha Failure**\n\n"
            "**Analysis**:\n"
            "- Threat: Malware (TRITON variant) detected on ICS controllers.\n"
            "- Impact: 100% loss of output (0 MW).\n"
            "- Risk: Cascading failure to North Sector Hospital Grid.\n\n"
            "**Recommended Actions**:\n"
            "1. **Isolate**: Disconnect Station Alpha from Main Grid immediately. [Auto-Executed]\n"
            "2. **Mitigate**: Reroute 450 MW from Station Beta to stabilize North Sector using 'Grid-7-Bypass'.\n"
            "3. **Respond**: Dispatch 'Cyber Response Unit 1' (ETA: 8 mins) to physical site.\n"
        )
        structured_data = {
            "alert_level": "CRITICAL",
            "impact_assessment": {"mw_lost": 450, "affected_assets": ["Station Alpha"]},
            "recommended_workflow": "isolate_and_reroute"
        }
        return {
            "text": ai_text,
...
# --- AIP Logic Endpoint ---
from nexus_os.apps.aip.logic_engine import logic_engine, LogicGraph

@app.post("/aip/logic/run")
async def run_aip_logic(
    ...
    """
    Execute an AIP Logic DAG.
    """
    try:
        # Default to ADMIN for now, ideally get from auth context
        result = await logic_engine.execute(graph, inputs, UserRole.ADMIN)
        return result
    except Exception as e:
...
def chat_endpoint(request: ChatRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        system_prompt = (
            f"You are Nexus AI, an advanced enterprise assistant for Palantir Nexus OS.\n\n"
            f"Your Role: You are assisting a user with the role: {user_role.value}.\n"
            "You must only answer questions and perform actions relevant to this role.\n\n"
            "[CURRENT ALERT STATUS: CRITICAL]\n"
            "- SITE: Station Alpha (Power Generation)\n"
            "- STATUS: OFFLINE (0 MW Output)\n"
            "- CAUSE: Suspected Cyber-Kinetic Attack (TRITON Malware signatures) on ICS Port 502.\n"
            "- RECOMMENDED PROTOCOLS:\n"
            "  1. ISOLATE Station Alpha from the main grid immediately.\n"
            "  2. REROUTE power from Station Beta to North Sector.\n"
            "  3. DISPATCH Cyber Response Unit 1 to the physical location.\n\n"
            "Context Information:\n"
            f"{context_str}\n\n"
            "Instructions:\n"
            "- If the user asks about 'Station Alpha' or 'Crisis', PRIORITIZE the Critical Alert info above.\n"
...
def list_objects(
    ...
    try:
        if object_type_id:
            objs = db.query(models.ObjectInstance).filter(models.ObjectInstance.object_type_id == object_type_id).limit(limit).all()
        else:
            objs = db.query(models.ObjectInstance).limit(limit).all()
