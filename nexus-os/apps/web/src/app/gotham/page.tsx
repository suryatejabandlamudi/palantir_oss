...
import { Map, Layers, Search, Filter, Ship, Anchor, Play, Square, Save, Radio } from 'lucide-react';
import { Button, Input } from '@nexus/ui';
import { getObjects, getObjectTypes } from '@/lib/api';
...
export default function GothamPage() {
    ...
    async function loadData() {
        try {
            ...
            const portType = types.find((t: any) => t.api_name === 'port');
            const vesselType = types.find((t: any) => t.api_name === 'vessel');

            if (portType) {
                const portData = await getObjects(portType.id);
                setPorts(portData);
            }
            if (vesselType) {
                const vesselData = await getObjects(vesselType.id);
            ...
            sidebar={
                <div className="flex flex-col h-full">
                    ...
                    <div className="p-4 flex-1 overflow-y-auto">
                        ...
                        <LayerItem name="Ports" active count={ports.length} icon={<Anchor className="w-3 h-3" />} />
                        <LayerItem name="Vessels" active count={vessels.length} icon={<Ship className="w-3 h-3" />} />
                        ...
                        {loading ? (
                            ...
                            <div className="space-y-1">
                                {ports.slice(0, 5).map(p => (
                                    <ObjectItem key={p.id} title={p.title} type="Port" onClick={() => setSelectedObject(p)} />
                                ))}
                                {vessels.slice(0, 5).map(v => (
                                    <ObjectItem key={v.id} title={v.title} type="Vessel" onClick={() => setSelectedObject(v)} />
                                ))}
                            </div>
                        )}
                    </div>
            ...
            <div className="h-full flex flex-col">
                ...
                <div className="flex-1 bg-[#1a2332] relative overflow-hidden">
                    ...
                    <div className="absolute bottom-4 right-4 bg-black/50 p-2 rounded text-xs text-gray-400 pointer-events-none flex items-center gap-2">
                        ...
                        {isLive ? "Live Feed Active" : "Historical View"} • {ports.length} Ports • {vessels.length} Vessels
                    </div>
                </div>
            </div>
        </WorkspaceLayout>
